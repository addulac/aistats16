...
\end{equation}
Where  $p(d^N=n)$ is the probability of a random graph with $N$ vertices and having $n$ edges. Thus it refers to the density of the graph. We now define the set of  adjacency matrix $Y_r$ corresponding to the ensemble of graphs having $n$ edges (and $N$ vertices) $Y_r \in \{Y: d=n\}$. Because of the exchangeability of the models, each of those adjacency matrices have the same distribution given $\Theta$, and the number of different graph that occurs are $\dbinom{N}{C(Y_r)}$, with $C(Y_r)$ is the number of node of the $Y_r$ having non empty degree:
\begin{equation}
p(y_{ij}^{new}=1| |V|=N) =  \int_{\Theta} p(y_{ij}^{new}=1|\Theta) \frac{\sum_{n=0}^{\dbinom{N}{2}} \sum_{Y_r \in \{Y: d=n\}}\dbinom{N}{C(Y_r)} p(Y_r| \Theta)}{\int_{\Theta} \sum_{n=0}^{\dbinom{N}{2}}  \sum_{Y_r \in \{Y: d=n\}}\dbinom{N}{C(Y_r)} p(Y_r | \Theta)} p(\Theta) d\Theta
\end{equation}

%\begin{equation}
%p(y_{ij}^{new}=1| |V|=N) =  \int_{\Theta} p(y_{ij}^{new}=1|\Theta) %\frac{\sum_{|\textbf{exG}|_{ij}} \textrm{comb}(\textbf{exG}_{ij}) p(Y_{\textrm{exG}_{ij}}| %\Theta)}{\int_{\Theta} \sum_{|\textbf{exG}_{ij}|} \textrm{comb}(\textbf{exG}_{ij}) %p(Y_{\textrm{exG}_{ij}} | \Theta)} p(\Theta) d\Theta
%\end{equation}
