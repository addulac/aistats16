...
\begin{equation}
p(y_{ij}^{new}=1| |V|=N) =  \int_{\Theta} p(y_{ij}^{new}=1|\Theta) \frac{\sum_{d=0}^{N} \sum_{n=0}^{\dbinom{N-1}{2}} p(Y_d^n| \Theta)}{\int_{\Theta}\sum_{d=0}^{N} \sum_{n=0}^{\dbinom{N-1}{2}} p(Y_d^n | \Theta)} p(\Theta) d\Theta
\end{equation}
